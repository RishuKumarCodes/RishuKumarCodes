<a href="http://rishukumarcodes.github.io/Portfolio_2.0">
  <img style="border-radius:25px" alt="Top Langs" width="100%" src="./img/github-bg.png">
</a>

<br><br>

<!-- Top projects section -->
<p style="font-weight:600; font-size:2rem; color:white; padding-left:30px">
  My top projects
</p>

<table align="center">
  <tr>
    <!-- Card: EduHaven -->
    <td style="min-width:300px; background-color:#fbfcfa15; border-radius:25px; padding:10px;">
      <a href="https://github.com/amandollar/EduHaven">
        <img src="https://github-readme-stats.vercel.app/api/pin/?username=amandollar&repo=eduHaven&bg_color=00000000&hide_border=true&theme=radical&title_color=ffffff&text_color=b8b8b8&icon_color=ffffff" 
             alt="eduHaven" 
             style="max-width:100%; transform:scale(108%)">
        <br>
        <img src="./img/EduHaven.png" 
             style="width:96.5%; padding:0 7px; border-radius:8px 8px 25px 25px;" 
             alt="EduHaven Screenshot">
      </a>
    </td>
    <td>
    <!-- Card: Stay-Sphere -->
    <td style="min-width:300px; background-color:#fbfcfa15; border-radius:25px; padding:10px;">
      <a href="https://github.com/rishukumarcodes/stay-sphere">
        <img src="https://github-readme-stats.vercel.app/api/pin/?username=RishuKumarCodes&repo=Stay-Sphere&bg_color=00000000&hide_border=true&theme=radical&title_color=ffffff&text_color=b8b8b8&icon_color=ffffff" 
             alt="Stay-Sphere" 
             style="max-width:100%; transform:scale(108%)">
        <br>
        <img src="./img/Stay-sphere.png" 
             style="width:96.5%; padding:0 7px; border-radius:8px 8px 25px 25px;" 
             alt="Stay-Sphere Screenshot">
      </a>
    </td>
  </tr>
</table>
<br><br>
<!-- Languages and Tools -->
<div style="border:1px solid #525252; border-radius:25px; padding:3 0px 45px; width:100%; box-sizing:border-box; font-weight:600; font-size:medium; color:white">
  Languages and Tools
  <br><br>
  <img src="https://skillicons.dev/icons?i=c,cpp,java,html,css,js,mysql,mongo,nodejs,react,nextjs,express,tailwind,bootstrap,git,github,vscode,ai,ps,pr,figma,linux&perline=13" 
       style="max-width:100%; height:auto" 
       alt="Skills">
</div>
<br>
<!-- Github status -->
<div style="border:1px solid #525252; border-radius:25px; padding:20px 10px; width:100%; box-sizing:border-box">
  <p align="center">
    <img alt="Rishu's GitHub stats" width="406" src="https://github-readme-stats.vercel.app/api?username=rishukumarcodes&custom_title=Github+Stats&bg_color=00000000&hide_border=true&show_icons=true&text_color=b8b8b8&title_color=FFFFFF&icon_color=25D366">
    <img alt="Top Langs" width="350" src="https://github-readme-stats.vercel.app/api/top-langs/?username=rishukumarcodes&layout=compact&hide_border=true&bg_color=00000000&text_color=b8b8b8&custom_title=Top+Languages&title_color=FFFFFF">
  </p>
</div>
<br><br>

### _Feel free to explore my repos & reach out if you'd like to collaborate!_
